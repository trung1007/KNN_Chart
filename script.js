...
document.querySelector('#predict').addEventListener('click', () => {
    ...
    for (let k = 1; k <= 5; k++) {
        ...
        chart.data.datasets[0].pointBackgroundColor[index] == 'red' ? redNeigbors++ : blueNeighbors++
        console.log(chart.data.datasets[0].pointBackgroundColor[index]);
        
        distances[index] = +Infinity
    }

    if(redNeigbors>blueNeighbors){
        document.querySelector('#output').innerHTML = "Most probably is will not rain"
        chart.data.datasets[0].pointBackgroundColor[chart.data.datasets[0].data.length - 1] = 'red'
    }
    else if(redNeigbors<blueNeighbors){
        document.querySelector('#output').innerHTML = "Most probably is will rain"
